...
export abstract class AbstractService {
  ...
  protected static readDB<FileT, OutT>(mapper: (item: FileT) => OutT): OutT[] {
    ...
    // }
    // return items;
    return dbos.map(mapper); // Modifié (programmation fonctionnelle)
  }

  protected static writeDB<From, ToWrite>(items: From[], mapper: (item: From) => ToWrite): boolean {
    const dbos: ToWrite[] = items.map(mapper); // Modifié (programmation fonctionnelle)
  ...
  protected static getNextId(items: { id: number }[]): number {
    ...
    return Math.max(...items.map((item) => item.id)) + 1; // Modifié (programmation fonctionnelle)
  }
}
