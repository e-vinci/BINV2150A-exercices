...
export class RecipesMapper {
  static toDTO(recipe: Recipe): RecipeDTO {
    const dto: RecipeDTO = {
      ...
      ...(recipe.imageUrl != null && { imageUrl: recipe.imageUrl }), // changé (spread conditionnel)
    };
  ...
  static fromNewDTO(dto: NewRecipeDTO, authorId: number): NewRecipe {
    return {
      ...
      tags: dto.tags ?? [], // changé (nullish coalescing)
  ...
  static fromDBO(dbo: RecipeDBO): Recipe {
    return {
      ...
      tags: dbo.tags ?? [], // changé (nullish coalescing)
      ingredients: dbo.ingredients ?? [], // changé (nullish coalescing)
      steps: dbo.steps ?? [], // changé (nullish coalescing)
