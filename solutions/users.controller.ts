...
usersController.get("/", AuthService.authorize, AuthService.isAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  const usersDTO = users.map(us => UsersMapper.toDTO(us));
...
usersController.get("/me/favorites", AuthService.authorize, (req: AuthenticatedRequest, res: Response) => {
  ...
  const recipes = RecipesService.getByIds(UsersService.getByEmail(user.email)?.favorites!);      // add a !, should ever work
  const recipesDTO = recipes.map(rec => RecipesMapper.toDTO(rec));
